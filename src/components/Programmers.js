...
export default function Programmers() {
  ...
  const [listOfProgrammers, setListOfProgrammers] = useState(listOfAwesome);
  ...
  const getNameOfFeatured = () => {
    ...
    const devObj = listOfProgrammers.filter(function(item) {
      return item.id === programmerId;
    })
    return devObj[0].name;
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: `${!programmerId ? 'royalblue' : 'gold'}`, // 🤔 color turns to gold, when celebrating
  };
